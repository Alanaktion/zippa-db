...
**Grid editing.** `ui::data_grid::DataGrid` keeps three overlays on top of the result it was handed: `edits` (typed cells, keyed by result row and column), `drafts` (rows being built by hand, each holding only the columns typed into, rendered after the result's rows and written by an `INSERT` so untouched columns keep the server's default), and `rows_selected` (a row-level selection swept with the mouse, separate from the table component's own selected cell). The grid never writes: it emits `GridEdit` and `TableView` turns the overlays into `UPDATE`/`INSERT`/`DELETE`. Deletes always go through the confirm panel, whatever the connection's `SafetyMode`.

The row menu is hung on the grid's own cell element (`render_td` → `.context_menu(...)`) *as well as* on `TableDelegate::context_menu`. With `cell_selectable(true)` a right click on a cell is answered by the table's own cell handler, which clears `right_clicked_row` and stops propagation, so the delegate's menu comes out empty there — and an empty `PopupMenu` is never shown, which is what keeps the two from both appearing. The delegate's menu covers the row-header column and the space beside the last column. Both build their items in `ResultDelegate::context_menu`.

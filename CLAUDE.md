...
**Persistence split.** `db/store.rs`: connection *metadata* goes to `connections.json` in the OS config dir; passwords go to the OS keychain via the `keyring` crate, keyed by connection `Uuid`. Never write a password into the JSON store. `ConnectionConfig::safety` (a `SafetyMode`) rides along in that metadata and decides whether inline grid edits are written when the selection leaves the row or wait to be applied by hand; it is `#[serde(default)]` so older files read back as the careful mode.

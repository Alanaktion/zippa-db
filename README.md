...
To avoid having to ship the app binary with a bunch of XML files that have to be installed to a specific location to work, we're packing the .ui files directly into the application with [packr2](https://github.com/gobuffalo/packr/tree/master/v2). This allows us to work off of local files during development, but build the app as a single file for distribution (see `build.sh` for how that works).

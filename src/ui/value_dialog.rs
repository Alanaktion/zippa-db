...
//! [`Workspace`](crate::app::Workspace) picks it up while it renders.
...
use gpui_kit::{
    App, Context, Entity, EventEmitter, Global, MouseButton, SharedString, Window, actions, div,
    hsla, rems,
};
...
impl Render for ValueView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // The backdrop dims the window and swallows what is clicked through
        // it, which is what makes this a dialog rather than a panel.
        div()
            .id("value-dialog")
            .absolute()
            .inset_0()
            ...
            .child(
                v_flex()
                    ...
                    .child(
                        // The box fills the dialog, so it grows with the
                        // window the dialog sits in.
                            ...
                            .when(editable, |this| {
                                this.child(
                                    Button::new("save-value")
                                        ...
                                        .on_click(
                                            cx.listener(|this, _, _window, cx| this.save(cx)),
                                        ),
                                )
                            }),
                    ),

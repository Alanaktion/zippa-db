...
/// Every theme set bundled under `assets/themes` — Zippa's own, built from
/// `assets/colors.md`, plus whatever else is dropped in there. Generated by
/// `build.rs`, so a new file just needs to exist to be picked up.
const BUILTIN_THEMES: &[&str] = include!(concat!(env!("OUT_DIR"), "/builtin_themes.rs"));

/// Names of Zippa's own theme, used as the out-of-the-box default.
const ZIPPA_DARK: &str = "Zippa Dark";
const ZIPPA_LIGHT: &str = "Zippa Light";
...
impl Settings {
    ...
    /// The theme in force for `mode`: the user's own pick, or Zippa's
    /// branded theme when they have not chosen one — an unset field in an
    /// older settings file reads back the same way, so an upgrade picks up
    /// the branded theme rather than staying on the plain built-in default.
    fn theme_for(&self, mode: ThemeMode) -> &SharedString {
        static ZIPPA_LIGHT_NAME: LazyLock<SharedString> = LazyLock::new(|| ZIPPA_LIGHT.into());
        static ZIPPA_DARK_NAME: LazyLock<SharedString> = LazyLock::new(|| ZIPPA_DARK.into());

        let chosen = match mode {
            ThemeMode::Light => self.light_theme.as_ref(),
            ThemeMode::Dark => self.dark_theme.as_ref(),
        };
        chosen.unwrap_or(match mode {
            ThemeMode::Light => &ZIPPA_LIGHT_NAME,
            ThemeMode::Dark => &ZIPPA_DARK_NAME,
        })
    }
}
...
/// Register every theme set bundled under `assets/themes`, so they are there
/// for the default above and for the picker in the settings window. Call
/// before [`init`], same as [`load_user_themes`].
pub fn load_builtin_themes(cx: &mut App) {
    for theme in BUILTIN_THEMES {
        if let Err(error) = ThemeRegistry::global_mut(cx).load_themes_from_str(theme) {
            eprintln!("could not load a built-in theme: {error:#}");
        }
    }
}
